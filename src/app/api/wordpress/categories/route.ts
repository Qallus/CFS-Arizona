...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${WP_REST_API_URL}/categories?per_page=100`, { headers });
    
    if (!res.ok) {
      const errorText = await res.text();
      return NextResponse.json({ error: 'Failed to fetch categories', details: errorText }, { status: res.status });
    }
